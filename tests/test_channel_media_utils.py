...
from copaw.app.channels.media_utils import (
    classify_media_kind,
    pick_attachment_url,
)
...
def test_classify_media_kind_by_mime_and_suffix() -> None:
    assert (
        classify_media_kind(mime_type="image/png", filename="x.bin") == "image"
    )
    assert classify_media_kind(mime_type="", filename="x.mp4") == "video"
    assert (
        classify_media_kind(mime_type="audio/mpeg", filename="x.bin")
        == "audio"
    )
