...
def test_launchd_stop_uses_service_target_bootout(
    ...
    monkeypatch.setattr("copaw.cli.service_cmd._launchd_label", lambda: "ai.copaw.app")
    monkeypatch.setattr("copaw.cli.service_cmd._launchd_domain", lambda: "gui/501")
    _launchd_stop()
...
def test_probe_version_bypasses_proxy_for_localhost(monkeypatch) -> None:
    ...
    monkeypatch.setattr("copaw.cli.service_cmd.build_opener", fake_build_opener)
...
def test_probe_version_retries_until_success(monkeypatch) -> None:
    ...
    monkeypatch.setattr("copaw.cli.service_cmd.build_opener", lambda *a, **k: _Opener())
...
def test_launchd_start_retries_bootstrap_then_succeeds(monkeypatch, tmp_path) -> None:
    commands: list[list[str]] = []
    ...
    def fake_run(cmd: list[str], *, check: bool = True) -> subprocess.CompletedProcess[str]:
        ...
        if cmd[:2] == ["launchctl", "bootstrap"]:
            ...
            if bootstrap_calls["count"] == 1:
                return subprocess.CompletedProcess(cmd, 5, "", "Input/output error")
            return subprocess.CompletedProcess(cmd, 0, "", "")
        return subprocess.CompletedProcess(cmd, 0, "", "")
    ...
    assert commands[0] == ["launchctl", "kickstart", "-k", "gui/501/ai.copaw.app"]
