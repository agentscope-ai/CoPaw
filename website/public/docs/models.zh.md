...
1. 打开控制台，进入 **设置 → 模型**。
2. 找到目标云提供商卡片（以 DashScope 为例），点击 **设置**。输入你的 **API key**，如有需要可额外填写 **额外请求体（extra_body）** JSON，然后点击 **保存**。
...
4. 点击设置，填写 **Base URL** 和 **API Key**。如果你的 OpenAI 兼容服务还要求额外请求体参数（例如 `thinking`、`cache_prompt`），也可以填写 **额外请求体（extra_body）** JSON，然后点击 **保存**。
