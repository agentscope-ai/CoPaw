...
| **uv tool 安装** | 已经使用 uv 的用户       | 快速隔离安装，自动提供命令入口 | uv               |
...
如果你已经在使用 [uv](https://docs.astral.sh/uv/)，可以将 QwenPaw 安装为隔离的命令行工具：

```bash
uv tool install qwenpaw
```

该命令会把 `qwenpaw` 安装到 uv 的工具目录中，避免污染项目虚拟环境。若安装后找不到命令，请运行 `uv tool update-shell`，或重新打开终端，让 uv 的工具目录生效到 `PATH`。
