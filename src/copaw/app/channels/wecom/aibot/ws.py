...
try:
    ...
    # websockets 14+ moved to websockets.asyncio; try new path first.
    try:
        from websockets.asyncio.client import ClientConnection as _WsConn
    except ImportError:
        # websockets <= 13.x
        from websockets.client import WebSocketClientProtocol as _WsConn  # type: ignore[no-redef]

    def _ws_is_open(ws) -> bool:
        """Compatible state check for websockets 12–16."""
        if hasattr(ws, 'open'):
            return ws.open  # type: ignore[attr-defined]
        if hasattr(ws, 'state'):
            try:
