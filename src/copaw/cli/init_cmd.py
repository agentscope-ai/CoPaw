...
def init_cmd(
    ...
    if health.status == HealthStatus.HEALTHY:
        click.secho("✓ All checks passed! You're ready to use CoPaw.", fg="green", bold=True)
        click.echo("\nNext steps:")
        click.echo("  • Run 'copaw app' to start the web console")
        click.echo("  • Visit http://localhost:8088 in your browser")
    elif health.status == HealthStatus.DEGRADED:
        click.secho(f"⚠ System is degraded ({health.degraded_count} warnings)", fg="yellow", bold=True)
        click.echo("\nYou can still use CoPaw, but some features may not work.")
        click.echo("Run 'copaw health' anytime to check system status.")
    else:
        click.secho(f"✗ System has critical issues ({health.unhealthy_count} errors)", fg="red", bold=True)
