...
`desktop/` 目录包含基于 Electron + TypeScript + Vite 构建的 **macOS 桌面客户端**。它内置独立的 Python 运行时，并以子进程方式启动 CoPaw 后端。

- **技术栈：** Electron Forge、Vite、TypeScript。
- **构建前置依赖：** [uv](https://github.com/astral-sh/uv)（Python 工具链）和 Node.js/npm。
- **构建命令：** 在仓库根目录运行 `bash scripts/desktop_build.sh`。
- **开发模式：** `cd desktop && npm ci && npm run dev`（需要 `desktop/runtime/` 中已存在内置运行时）。

欢迎以下方向的贡献：
- **多平台支持：** 桌面客户端目前仅支持 macOS。添加 Windows 和 Linux 打包（如 NSIS 安装器、AppImage/deb）是非常有价值的贡献。
- **原生集成：** 系统托盘、通知、自动更新、深度链接。
- **用户体验改进：** 启动画面、引导流程、菜单操作。
