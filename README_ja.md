...
- [2026-05-19] **v1.1.8** をリリースしました！詳細は [v1.1.8 リリースノート](https://qwenpaw.agentscope.io/release-notes) をご覧ください。
  ...
  - **[v1.1.8] 新機能**：公式プラグインリソース（Web サイトからダウンロード + コンソールからワンクリックインストール）；CloudPaw アリババクラウドデプロイプラグイン；QwenPaw Pet デスクトップペット；`/make-skill` コマンド；カスタム HTTP ヘッダーと認証モード；モデル別トークン制限；受信トレイ一括操作；チャット履歴ドロワー固定。
