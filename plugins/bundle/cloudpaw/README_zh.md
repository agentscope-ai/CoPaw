...
CloudPaw 是 QwenPaw 的阿里云AI助手插件，融合 **QwenPaw + Aliyun CLI** 两大核心组件，并深度集成 **ROS（资源编排）** 能力——它不是简单的聊天机器人，而是一个具备云原生执行引擎的智能助手。
...
2. 点击左侧导航栏的**「插件管理」**（设置分组下） → 点击**「安装插件」**
3. 输入 `cloudpaw/` 文件夹路径，或将其拖拽到安装对话框中
4. 等待安装完成
...
CloudPaw 依赖 [iac-code](https://github.com/aliyun/iac-code) 生成 IaC 模板。首次运行 `iac-code` 命令会自动引导配置，也可手动编辑 `~/.iac-code/settings.yml`。

### 3. 开始使用

在控制台聊天页面的 **Agent 下拉框**中选择**「CloudPaw-Master」**，即可开始对话。
...
- **ROS 部署编排**：通过 [iac-code](https://github.com/aliyun/iac-code) 引擎自动生成 ROS/Terraform 模板，实现阿里云资源自动化部署
