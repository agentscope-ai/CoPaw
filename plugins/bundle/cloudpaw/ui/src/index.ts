...
function patchA2aSlashCommand() {
  ...
  function extractA2aSlash(text: string): { alias: string; cleanMsg: string } | null {
    ...
    if (Array.isArray(content)) {
      return content.map((item: any) => {
        if (item.type === "text" && typeof item.text === "string") {
          ...
          if (info && info.cleanMsg) {
            return { ...item, text: A2A_HINT_TEMPLATE(info.alias, info.cleanMsg) };
          }
        }
        return item;
      });
  ...
  window.fetch = async function (input: any, init?: RequestInit): Promise<Response> {
