# -*- coding: utf-8 -*-
"""Manage PRD tool — create, add, update, delete, mark_passed stories in prd.json."""
...
def _validate_story(story: dict, index: int = -1, check_passes_notes: bool = True) -> tuple[bool, str]:
    if check_passes_notes:
        missing = _REQUIRED_STORY_FIELDS - set(story.keys())
    else:
        # create 操作不需要 story 中有 passes/notes，会自动填充
        required_fields = frozenset({"id", "title", "description", "acceptanceCriteria", "priority"})
    ...
    if not isinstance(acceptance, list) or len(acceptance) == 0:
        return False, f"story '{story.get('id', '?')}' acceptanceCriteria 必须是非空字符串数组"
...
async def manage_prd(
    ...
    Args:
        ...
        operation (`str`):
            Operation type: "create" | "add" | "update" | "delete" | "mark_passed".
    ...
    if operation.lower().strip() == "create":
        if prd_path.exists():
            try:
                ...
                if isinstance(existing, dict) and isinstance(existing.get("userStories"), list):
        ...
        return await _handle_create(loop_path, prd_path, project, description, branch_name, stories)
    ...
    if operation not in valid_operations:
        return _error_response(f"无效操作: '{operation}'。允许: {', '.join(valid_operations)}")
    ...
    stories = prd.get("userStories") or prd.get("stories", [])
    ...
    if operation == "add":
        ...
        if story["id"] in {s.get("id") for s in stories}:
        ...
        prd["userStories"] = [s for s in stories if s.get("id") not in story_ids]
        _save_prd(prd_path, prd)
        deleted = sum(1 for sid in story_ids if sid in existing_ids)
